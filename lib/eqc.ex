defmodule EQC do
  ...
  The variables of `pat` are bound in `gen_a`, and, similarly, the variables of `pat1` and `pat2`
  are bound in `gen_b`.
  ...
  defmacro such_that_maybe(_, _) do
  ...
  defmacro implies(pre, do: prop) when prop != nil do
    quote do
      :eqc.implies(unquote(pre), unquote(to_char_list(Macro.to_string(pre))), fn ->
        unquote(prop)
      end)
    end
  end
  defmacro implies(_, _) do
  ...
  defmacro setup_teardown(_, _) do
    ...
    acc = quote do
      :eqc.collect(fn res ->
        case (unquote(requirement) -- Keyword.keys(res)) do
          [] -> :ok
          uncovered -> :eqc.format("Warning: not all features covered! ~p\n", [uncovered])
        end
